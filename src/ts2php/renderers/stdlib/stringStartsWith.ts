...
export const stringStartsWith: ExpressionHook = (node: ts.CallExpression, context: Context<Declaration>) => {
  ...
  if (!args || !args[0]) {
    context.log.error('String.prototype.startsWith: can\'t find searchable element in call.', [], ctx(node));
    return 'null';
  }
  context.log.warn('Converting String.prototype.startsWith to strpos(): check your encodings twice!', [], ctx(node));
  if (args[1]) {
