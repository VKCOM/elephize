...
export const arrayForeach: ExpressionHook = (node: ts.CallExpression, context: Context<Declaration>) => {
  ...
  if (!funcBlockNode) {
    context.log.error('Array.prototype.forEach: can\'t find callable argument in call.', [], ctx(node));
    return 'null';
  }
  ...
  if (!cbArgs || !cbArgs[0]) {
    context.log.error('Array.prototype.forEach: can\'t find iterable element in call.', [], ctx(node));
    return 'null';
  }

  if (cbArgs[0].name.kind !== ts.SyntaxKind.Identifier) {
    context.log.error('Array.prototype.forEach: parameter destructuring in foreach is not supported.', [], ctx(node));
    return 'null';
  }
  if (cbArgs[1]) {
