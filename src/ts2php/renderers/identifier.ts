...
export function tIdentifier(node: ts.Identifier, context: Context<Declaration>) {
  ...
  if (context.dryRun && node.escapedText === 'window' && node.parent.getText() === 'window._elephizeIsServer') {
    context.log.error('Special variable \'window._elephizeIsServer\' should be used in ternary conditions only!', [], ctx(node));
  }
  ...
  if (node.escapedText.toString() === 'require') {
    context.log.error('You should use `import` instead of `require`', [], ctx(node));
    return 'null';
  }
  ...
  if (!context.dryRun && isDeclaration && !context.scope.checkUsage(node.getText())) {
    ...
    context.log.info('Dropped unused var $%s from [out]/%s', [node.escapedText.toString(), context.moduleDescriptor.targetFileName], ctx(node));
