...
function tIdentifier(node, context) {
    ...
    if (context.customGlobals[node.escapedText.toString()]) {
        if (decl && scope && scope.tNodeLocal !== usageGraph_1.Scope.tNode) {
            ...
            log_1.log("Replacing variable according to customGlobals setting: " + node.escapedText.toString() + " -> " + context.customGlobals[node.escapedText.toString()], log_1.LogSeverity.INFO, log_1.ctx(node));
