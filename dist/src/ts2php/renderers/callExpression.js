...
function tCallExpression(node) {
    return {
        ...
        gen: function (self, context) {
            ...
            var hookResult = stdlib_1.hookStdlib(node, self, context);
            if (hookResult !== undefined) {
                return hookResult;
            }
            var identNode = children[0], syntaxList = children[2];
            var ident = identNode.node.gen(identNode, context);
            var args;
            var reactHooks = reactHooks_1.reactHooksSupport(context, node, self);
            if (reactHooks) {
                if (reactHooks === '!null') {
            ...
            if (ident === '$this->h') {
                ...
                var _a = syntaxList.children, componentName = _a[0], attrs = _a[2], children_1 = _a.slice(4);
                ...
                if (componentName.node.kind === ts.SyntaxKind.StringLiteral) { // intrinsic element
                    componentNameRendered = renderSupportedNodes_1.renderSupportedNodes([componentName], context)[0];
                    ...
                    attrsRendered = renderSupportedNodes_1.renderSupportedNodes([attrs], context)[0] || '[]';
                    childrenRendered = renderSupportedNodes_1.renderSupportedNodes(children_1, context);
                }
                ...
                ast_1.flagParentOfType(self, [ts.SyntaxKind.IfStatement], { drop: true });
                ast_1.flagParentOfType(self, [ts.SyntaxKind.ForInStatement], { validated: true });
                args = renderSupportedNodes_1.renderSupportedNodes(__spreadArrays(syntaxList.children), context);
            }
            else {
                args = renderSupportedNodes_1.renderSupportedNodes(__spreadArrays(syntaxList.children), context);
            }
