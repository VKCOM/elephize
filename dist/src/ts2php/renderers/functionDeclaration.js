        ...
        if (node.name) {
            ...
            if (els) { // should be true for all non-components, this check is only for typescript to be happy
                ...
                if (!context.dryRun && context.scope.checkUsage(node.name.getText())) {
                    context.moduleDescriptor.addMethod(node.name.getText(), block, syntaxList.join(', '), _assert_1.getPhpPrimitiveTypeForFunc(node, syntaxList, context.checker), 'public');
                }
